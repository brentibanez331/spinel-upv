...
export async function processUserMessage({
  ...
  try {
    ...
    relevantDocs.forEach((doc, i) => {
      console.log(
        `Doc ${i + 1} preview: ${doc.pageContent.substring(0, 100)}...`
      );
    });

    if (relevantDocs.length === 0) {
      console.warn("No relevant documents found in the vector store");
      // You might want to return a specific message or fallback behavior
    }
